...
# Keep runtime artifacts outside the bundled source tree so standalone builds
# can run from read-only locations.
RUNTIME_DIR = os.path.abspath(os.environ.get("CODEX_PYTHON_RUNTIME_DIR") or SCRIPT_DIR)
...
def _account_to_line(account):
    """Normalize a legacy line or a JSON credential object for auto_login.py."""
    if isinstance(account, str):
        line = account.strip()
        if not line:
            raise ValueError("Account entry is empty")
        return line
    if not isinstance(account, dict):
        raise ValueError("Each account must be a string or object")

    nested = account.get("credentials")
    sources = [account]
    ...
    email = first_value(("email", "username", "login"))
    password = first_value(("password",))
    totp = first_value((
        "totpSecret", "totp", "twoFactorSecret", "two_factor",
        "two_factor_secret", "otp", "totp_secret", "2fa", "2fa_secret",
    ))
    if not email or not password:
        raise ValueError("Each account object must include email and password")
    return "|".join((email, password, totp))


def _normalize_auto_login_accounts(accounts):
    """Accept the old list-of-lines and the JSON payload used by the Next UI."""
    if isinstance(accounts, str):
        accounts = accounts.splitlines()
    if not isinstance(accounts, list):
        raise ValueError("accounts must be a list or newline-separated string")
    normalized = [_account_to_line(account) for account in accounts]
    if not normalized:
        raise ValueError("No accounts")
    return normalized


def _auto_login_worker(accounts, headed=False, workers=3):
    ...
    accounts = _normalize_auto_login_accounts(accounts)
    active_workers = min(workers, len(accounts))
    tmp_file = os.path.join(RUNTIME_DIR, "_tmp_accounts.txt")
    # Keep the credential staging file private while the legacy worker reads it.
    ...
    try:
        ...
        # Put the worker and its Playwright browsers in their own process group
        # so Stop All can terminate the complete tree on POSIX as well as Windows.
        if os.name != "nt":
            popen_options["start_new_session"] = True
        proc = subprocess.Popen(args, **popen_options)
...
def start_auto_login(accounts, headed=False, workers=3):
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p == "/api/oauth/auto-login":
            ...
            try:
                data = json.loads(body)
                accounts = data.get("accounts", data.get("accountList", data.get("lines", [])))
                accounts = _normalize_auto_login_accounts(accounts)
                if not accounts:
                ...
                if workers < 1:
                    self._json({"error": "Workers must be a positive integer"}, 400)
                    return
                # Match auto_login.py: headless by default, with an explicit
                # headed/show-browser option for interactive Cloudflare/OAuth.
                headed_value = data.get("headed", data.get("showBrowser", False))
                if isinstance(headed_value, str):
                    headed = headed_value.strip().lower() in ("1", "true", "yes", "on", "headed")
                else:
                    headed = bool(headed_value)
...
def main():
    ...
    def handle_shutdown(signum, _frame):
        print("\nStopping Python auto-login backend (signal {})...".format(signum), flush=True)
        if _auto_login_status.get("running") or _auto_login_proc:
            stop_auto_login()
    ...
    # The Next.js adapter starts this process as a localhost sidecar.  In that
    # mode the dashboard already owns the UI, so opening the standalone import
    # page would be surprising and can steal focus from the user's browser.
    if os.environ.get("CODEX_PYTHON_SIDECAR") != "1":
        def open_browser():
            import time
            time.sleep(0.8)
